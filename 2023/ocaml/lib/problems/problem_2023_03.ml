...
module Part_1 = struct
  ...
  let lex st =
    String.foldi st ~init:[] ~f:(fun i out c ->
      ...
      then
        ...
        then out
          ...
          out
          @ [ String.fold_until
                (String.drop_prefix st i)
                ~init:{ idx = i; t_type = Num { value = 0; len = 0 } }
                ~f:(fun acc n ->
                  if not (Char.is_digit n)
                  then Stop acc
                  else (
                    let v, l =
                      match acc.t_type with
                    ...
                    Continue
                      { acc with
                        t_type =
                          Num { value = (v * 10) + Char.get_digit_exn n; len = l + 1 }
                      }))
                ~finish:(fun acc -> acc)
            ]
      else out)
  ...
  let gen_surrounding_indeces num idx ~row_len =
    (* left *)
    [ (if (idx - 1) mod row_len = row_len - 1
       then []
       else [ idx - 1; idx - row_len - 1; idx + row_len - 1 ])
    ; (* right *)
      (if (idx + num.len) mod row_len = 0
       then []
       else [ idx + num.len; idx - row_len + num.len; idx + row_len + num.len ])
  ...
  let is_part_num num idx ~row_len ~tokens =
    let symbol_indeces =
      List.filter_map tokens ~f:(fun t ->
        ...
        | Num _ -> None)
    in
    List.exists (gen_surrounding_indeces num idx ~row_len) ~f:(fun i ->
      List.mem symbol_indeces i ~equal:( = ))
  ;;
    ...
    Ok
      (let tokens = String.filter input ~f:(fun c -> not Char.(c = '\n')) |> lex in
       ...
       List.fold tokens ~init:0 ~f:(fun acc t ->
         ...
         | Symbol -> 0
         | Num num ->
           if is_part_num num t.idx ~row_len ~tokens
           then (
             Printf.printf "%d\n" num.value;
             num.value)
           else 0)
...
module Part_2 = struct
  ...
  let idx_in_num_token idx t =
    ...
    | PotentialGear -> false
    | Num num ->
      List.exists ~f:(fun idx' -> idx = idx') (List.init num.len ~f:(fun i -> t.idx + i))
  ;;

  let get_gear_ratio gear_idx ~in_ ~row_len =
    let surrounding = gen_surrounding_indeces gear_idx ~row_len in
    let adj =
      List.filter_map in_ ~f:(fun t ->
        match t.t_type with
        | PotentialGear -> None
        | Num num ->
          if List.exists surrounding ~f:(fun i -> idx_in_num_token i t)
          then Some num
          else None)
    in
    if List.length adj = 2
    then (List.nth_exn adj 0).value * (List.nth_exn adj 1).value
    else 0
  ;;
